...
public class ImGuiRenderer : IDisposable
{
    ...
    public void Render(Vector2i windowResolution)
    {
        ImGui.GetIO().DisplaySize =
            new System.Numerics.Vector2(windowResolution.X,
                windowResolution.Y); // divide by DisplayFramebufferScale if necessary
        ImGui.Render();
    ...
    private void CreateDeviceResources()
    {
        ...
        void SetAttribute(int index, int size, VertexAttribPointerType type, int relativeoffset,
            bool normalized = false)
        {
            int stride = Unsafe.SizeOf<ImDrawVert>();

            var type2 = type switch
            {
                VertexAttribPointerType.Float => VertexAttribType.Float,
                VertexAttribPointerType.UnsignedByte => VertexAttribType.UnsignedByte,
                _ => VertexAttribType.Float
            };

            GL.VertexAttribPointer(index, size, type, normalized, stride, relativeoffset);
            //GL.VertexAttribBinding(index, index);
         //   GL.VertexAttribFormat(index, size, type2, normalized, relativeoffset);
        ...
        SetAttribute(0, 2, VertexAttribPointerType.Float, 0);
        SetAttribute(1, 2, VertexAttribPointerType.Float, 8);
        SetAttribute(2, 4, VertexAttribPointerType.UnsignedByte, 16, true);
...
void main()
{
    ...
    private void RecreateFontDeviceTexture()
    {
        ...
        fontTexture?.Dispose();
        fontTexture = new(width, height, SizedInternalFormat.Rgba8, 1)
        {
            WrapMode = TextureWrapMode.Repeat, MinFilter = TextureMinFilter.Linear, MagFilter = TextureMagFilter.Linear
        };
        ...
        for (int n = 0; n < draw_data.CmdListsCount; n++)
        {
            ...
            for (int cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
            {
                ...
                else
                {
                    ...
                    GL.Scissor((int)clip.X, (int)io.DisplaySize.Y - (int)clip.W, (int)(clip.Z - clip.X),
                        (int)(clip.W - clip.Y));
